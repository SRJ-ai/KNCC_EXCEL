...
def test_full_flow():
    ...
    auth_data = {
        "email": "test_inprocess@kncc.com",
        "password": "testpassword123",
        "name": "Test User",
        "organization_name": "KNCC"
    }
    # Try register first
    res = client.post(f"{BASE_URL}/auth/register", json=auth_data)
    if res.status_code == 400 and "Email already registered" in res.text:
        ...
        res = client.post(f"{BASE_URL}/auth/login", data=login_data)
    ...
    res = client.post(f"{BASE_URL}/projects/", json={"name": "Willow Way Village", "job_number": "60126", "tax_rate": 1.06}, headers=headers)
    willow_id = res.json()["id"]
    
    res = client.post(f"{BASE_URL}/projects/", json={"name": "Cobia Cove Appartments", "job_number": "68981", "tax_rate": 1.06}, headers=headers)
    ...
    for project_id, directory in [(willow_id, WILLOW_DIR), (cobia_id, COBIA_DIR)]:
        ...
        for pdf_path in pdfs:
            if "po.pdf" in pdf_path.lower() or "purchase order" in pdf_path.lower():
                ...
                print(f"Uploading PO: {filename}...")
                res = client.post(f"{BASE_URL}/upload/confirm", data={
                    "filename": filename,
                    "doc_type": "PO",
                ...
                assert data.get("line_items_parsed", 0) > 0
    ...
    res = client.post(f"{BASE_URL}/export/{willow_id}", headers=headers)
    res.raise_for_status()
    res = client.post(f"{BASE_URL}/export/{cobia_id}", headers=headers)
    res.raise_for_status()
